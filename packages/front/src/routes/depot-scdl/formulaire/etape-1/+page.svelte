<script>
    ...
    const controller = new StepOneController();
</script>
...
<div class="fr-mb-6v">
    <Alert type="warning" title="Alerte sur l’écrasement des données">
        Pour garantir la qualité et l'exhaustivité des données, nous vous demandons de <strong>
            déposer un fichier unique contenant l’ensemble des années disponibles dans votre périmètre, y compris les
            données déjà transmises si elles doivent être maintenues.
        </strong>
...
<div>
    <button on:click={controller.goToDepositWelcomePage} class="fr-btn fr-btn--secondary fr-mr-3v" type="button">
        Retour
    </button>

    <button
        on:click={controller.goToStep2}
