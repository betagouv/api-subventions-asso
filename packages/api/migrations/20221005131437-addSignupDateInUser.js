    ...
    async up(db, client) {
        ...
        await db.collection('log').aggregate(
            [
                { $match: { 
                    $or: [
                        { "meta.req.url": new RegExp("/user/admin/create-user"), "meta.res.statusCode": 200 },
                        { "meta.req.url": new RegExp("/auth/signup"), "meta.res.statusCode": 201 }
                    ]
                }}, 
                { $group: {
        ...
        await asyncForEach(users, async (user) => await userRepository.update({...user, signupAt: new Date("2022-01-01")}));
    },
