...
describe("express.auth.hooks", () => {
    ...
    describe("local", () => {
        it("Should be logged user", (done) => {
            ...
            jest.spyOn(userService, 'login').mockImplementation((email) => Promise.resolve({success: true, user: { email, roles: [], active: true, jwt: { token: "", expirateDate: new Date()}, stats: { searchCount: 0 }}}) )
    ...
    describe("jwt", () => {
        it("Should be logged user", (done) => {
            ...
            jest.spyOn(userService, 'findByEmail').mockImplementationOnce((email) => Promise.resolve({ email, roles: [], active: true, jwt: { token: "", expirateDate: new Date()}, stats: { searchCount: 0 }, _id: new ObjectId()}) )
