...
describe("expressAuthentication", () => {
    ...
    beforeAll(() => {
        ...
        findByEmailMock.mockImplementation((email) => Promise.resolve({ email, roles: ["user"], active: true, signupAt: new Date(), stats: { searchCount: 0 } , _id: new ObjectId()}));
        ...
        updateMock.mockImplementation((user) => Promise.resolve({ email: user.email, roles: ["user", "admin"], active: true, signupAt: new Date(), stats: { searchCount: 0 } , _id: new ObjectId() }))
