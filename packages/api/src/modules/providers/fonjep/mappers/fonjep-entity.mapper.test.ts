...
describe("FonjepEntityAdapter", () => {
    ...
    describe("toFonjepTierEntity()", () => {
        ...
        it("should map FonjepTiersDto to FonjepTiersEntity correctly", () => {
            ...
            const actual = FonjepEntityMapper.toFonjepTierEntity(tier, EXPORT_DATE);
            expect(actual).toEqual(expected);
        });

        it("call removeWhitespace if siretOrRidet is defined", () => {
            const tier = TIER_DTOS[0];
            FonjepEntityMapper.toFonjepTierEntity(tier, EXPORT_DATE);
    ...
    describe("toFonjepPosteEntity()", () => {
        it("should map FonjepPosteDto to FonjepPosteEntity correctly", () => {
            ...
            const actual = FonjepEntityMapper.toFonjepPosteEntity(poste, EXPORT_DATE);
            expect(actual).toEqual(expected);
        });
    });
    ...
    describe("toFonjepVersementEntity()", () => {
        it("should map FonjepVersementDtoWithExcelDate to FonjepVersementEntity correctly", () => {
            ...
            const actual = FonjepEntityMapper.toFonjepVersementEntity(versement, EXPORT_DATE);
            expect(actual).toEqual(expected);
        });
    });
    ...
    describe("toFonjepTypePosteEntity()", () => {
        it("should map FonjepTypePosteDto to FonjepTypePosteEntity correctly", () => {
            ...
            const actual = FonjepEntityMapper.toFonjepTypePosteEntity(typePoste, EXPORT_DATE);
            expect(actual).toEqual(expected);
        });
    });
    ...
    describe("toFonjepDispositifEntity()", () => {
        it("should map FonjepDispositifDto to FonjepDispositifEntity correctly", () => {
            ...
            const actual = FonjepEntityMapper.toFonjepDispositifEntity(dispositif, EXPORT_DATE);
            expect(actual).toEqual(expected);
        });
    });
    ...
    describe("getBopFromFounderCode", () => {
        ...
        `("should return value", ({ code, expected }) => {
            const actual = FonjepEntityMapper.getBopFromFounderCode(code);
            expect(actual).toEqual(expected);
        });
    });
    ...
    describe("(flat part)", () => {
        ...
        describe("toFonjepPaymentFlat", () => {
            ...
            const mockGetBopFromFounderCode = jest.spyOn(FonjepEntityMapper, "getBopFromFounderCode");
            ...
            it("returns FonjepPaymentFlat entity", () => {
                const actual = FonjepEntityMapper.toFonjepPaymentFlat(
                    { payment: VERSEMENT_ENTITY, position: POSTE_ENTITY, thirdParty: TIERS_ENTITY },
                    DATA_BRETAGNE_RECORDS,
                );
        ...
        describe("extractPositionCode", () => {
            it("returns code poste", () => {
                ...
                const actual = FonjepEntityMapper.extractPositionCode(FONJEP_PAYMENT_FLAT_ENTITY);
                expect(actual).toEqual(expected);
            });

            it("throws error if not a FonjepPaymentFlatEntity", () => {
                // @ts-expect-error: edge case with wrong parameter type
                expect(() => FonjepEntityMapper.extractPositionCode(CHORUS_PAYMENT_FLAT_ENTITY)).toThrowError(
        ...
        describe("buildFlatPaymentId", () => {
            ...
            beforeEach(() => {
                ...
                mockGetConventionDate = jest
                    ...
                    .spyOn(FonjepEntityMapper as any, "getConventionDate")
            ...
            it("return idVersement", () => {
                ...
                const actual = FonjepEntityMapper.buildFlatPaymentId({
                    thirdParty: TIERS_ENTITY,
                    position: POSTE_ENTITY,
                });
                expect(actual).toMatchSnapshot();
            ...
            it("calls getConventionDate", () => {
                // @ts-expect-error: test private method
                FonjepEntityMapper.buildFlatPaymentId({
                    thirdParty: TIERS_ENTITY,
                    position: POSTE_ENTITY,
                });
        ...
        describe("getConventionDate", () => {
            ...
            it("calls modifyDateYear", () => {
                // @ts-expect-error: test private method
                FonjepEntityMapper.getConventionDate(POSTE_ENTITY);
            ...
            it("throws error if position has no dateFinTriennalite", () => {
                expect(() =>
                    // @ts-expect-error: test private method
                    FonjepEntityMapper.getConventionDate({ ...POSTE_ENTITY, dateFinTriennalite: undefined }),
        ...
        describe("buildPaymentFlatUniqueId", () => {
            ...
            it("return idVersement", () => {
                ...
                // @ts-expect-error: test private method
                const actual = FonjepEntityMapper.buildPaymentFlatUniqueId(partialPaymentFlat);
                expect(actual).toMatchSnapshot();
            });
        });
        ...
        describe("toFonjepApplicationFlat", () => {
            ...
            beforeEach(() => {
                ...
                mockBuildFlatPaymentId = jest
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    .spyOn(FonjepEntityMapper as any, "buildFlatPaymentId")
            ...
            it("build payment id", () => {
                FonjepEntityMapper.toFonjepApplicationFlat({
                    position: POSITION,
            ...
            it("return application flat entity", () => {
                const actual = FonjepEntityMapper.toFonjepApplicationFlat({
                    position: POSITION,
                    beneficiary: BENEFICIARY,
                    allocator: ALLOCATOR,
                    instructor: INSTRUCTOR,
