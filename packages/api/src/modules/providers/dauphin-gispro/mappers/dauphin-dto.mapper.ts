...
export default class DauphinDtoMapper {
    ...
    private static getStatus(rawStatus: string) {
        return toStatusFactory(DauphinDtoMapper._statusConversionArray)(rawStatus);
    }
    ...
    public static toDocuments(documentReferences: DauphinDocumentDto[]) {
        ...
        for (const reference of documentReferences) {
            ...
            for (const doc of reference.documents) {
                ...
                toPV = ProviderValueFactory.buildProviderValueMapper(dauphinService.meta.name, new Date(date));
                resultArray.push({
