...
describe("GrantAdapter", () => {
    ...
    describe("findSingleProperty", () => {
        ...
        it("if null payment arg returns undefined", () => {
            ...
            const actual = GrantMapper.findSingleProperty(null, PROPERTY, MULTIVALUE);
        ...
        it("if single value and no adapter, return it", () => {
            ...
            // @ts-expect-error -- test private
            const actual = GrantMapper.findSingleProperty([PAYMENT, PAYMENT, PAYMENT], PROPERTY, MULTIVALUE);
        ...
        it("if single value and adapter, return actual value", () => {
            ...
            // @ts-expect-error -- test private
            const actual = GrantMapper.findSingleProperty([PAYMENT, PAYMENT, PAYMENT], PROPERTY, MULTIVALUE, ADAPTER);
        ...
        it("if several values, return multivalue arg", () => {
            ...
            // @ts-expect-error -- test private
            const actual = GrantMapper.findSingleProperty(
    ...
    describe("grantToExtractLine", () => {
        ...
        beforeAll(() => {
            ...
            singlePropMock = jest.spyOn(GrantMapper, "findSingleProperty").mockReturnValue("aggregated");
            mockExtractExercise = jest.spyOn(GrantMapper, "extractExerciseFromGrant").mockReturnValue(2024);
            // @ts-expect-error -- mock
            addressToOneLineStringMock = jest.spyOn(GrantMapper, "addressToOneLineString").mockReturnValue("adresse");
            jest.mocked(paymentService.getPaymentExercise).mockReturnValue(2022);
        });

        it("adapts correctly", () => {
            const actual = GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET);
            expect(actual).toMatchSnapshot();
        });

        it("chooses last financial center", () => {
            const expected = `${LAST_PAYMENT.financialCenterCode} - ${LAST_PAYMENT.financialCenterLabel}`;
            const actual = GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET).financialCenter;
            expect(actual).toBe(expected);
        });

        it("chooses last payment date", () => {
            const expected = "2025-11-20";
            const actual = GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET).paymentDate;
            expect(actual).toBe(expected);
        });

        it("extract budgetary exercise", () => {
            GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET);
        ...
        it("paidAmount", () => {
            ...
            const actual = GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET).paidAmount;
            expect(actual).toBe(expected);
        });
        ...
        it("calls findSingleProperty for program", () => {
            ...
            const actual = GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET).program;
        ...
        it("use denomination_siren of no denomination_rna", () => {
            ...
            const actual = GrantMapper.grantToExtractLine(
                GRANT,
        ...
        it("gets postalCode", () => {
            ...
            const actual = GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET).postalCode;
            expect(actual).toBe(expected);
        });
        ...
        it("gets formatted address", () => {
            ...
            const actual = GrantMapper.grantToExtractLine(GRANT, ASSO, ESTAB_BY_SIRET).estabAddress;
