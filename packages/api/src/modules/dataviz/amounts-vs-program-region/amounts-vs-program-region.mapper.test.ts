...
} from "./__fixtures__/amounts-vs-program-region.fixture";
...
describe("AmountsVsProgramRegionAdapter", () => {
    describe("toDbo", () => {
        it("should return right mapping", () => {
            const actual = AmountsVsProgramRegionMapper.toDbo(AMOUNTS_VS_PROGRAM_REGION_ENTITIES[0]);
    ...
    describe("toEntity", () => {
        it("should return right mapping", () => {
            const actual = AmountsVsProgramRegionMapper.toEntity({
    ...
    describe("toNotAggregatedEntity", () => {
        it("should return right mapping when paymentFlat is without nulls", () => {
            const actual = AmountsVsProgramRegionMapper.toNotAggregatedEntity(CHORUS_PAYMENT_FLAT_ENTITY);
            const expected = EXPECTED_WITHOUT_NULLS;
            expect(actual).toEqual(expected);
        });

        it("should return right mapping when paymentFlat is with nulls", () => {
            const actual = AmountsVsProgramRegionMapper.toNotAggregatedEntity(CHORUS_PAYMENT_FLAT_ENTITY_WITH_NULLS);
