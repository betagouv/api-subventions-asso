...
describe("SubventionsService", () => {
  ...
  describe("getDemandesByAssociation()", () => {
    ...
    it("should return DemandeSubvention[]", async () => {
      getIdentifierTypeMock.mockImplementationOnce(
        () => StructureIdentifiersEnum.siren
      );
      const expected = [{}, {}];
      const flux = await subventionsService.getDemandesByAssociation(
        IDENTIFIER
      );
      const actual = (await flux.toPromise())
        .map((fs) => fs.subventions || [])
        .flat();
      expect(actual).toEqual(expected);
    });
  });
  ...
  describe("getDemandesByEtablissement", () => {
    ...
    it("should return DemandeSubvention[]", async () => {
      ...
      const expected = [{}, {}];
      const flux = await subventionsService.getDemandesByEtablissement(
        IDENTIFIER
      );
      const actual = (await flux.toPromise())
        .map((fs) => fs.subventions || [])
