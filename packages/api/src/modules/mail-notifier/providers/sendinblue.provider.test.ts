...
describe("sendinblue.provider", () => {
    ...
    describe("sendMail()", () => {
        ...
        it("should mock", async () => {
            ...
            const expected = [{ "bcc": [{ "email": "log@datasubvention.beta.gouv.fr", "name": "Data.Subvention Log" }], "htmlContent": HTML, "sender": { "email": process.env.MAIL_USER, "name": "Data.Subvention" }, "subject": SUBJECT, "to": [{ "email": EMAIL }] }];
