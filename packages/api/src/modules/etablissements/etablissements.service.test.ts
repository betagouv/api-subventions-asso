...
describe("EtablissementsService", () => {
  const toSimplifiedEtablissementMock = jest.spyOn(
    EtablissementAdapter,
    "toSimplifiedEtablissement"
  );
  //@ts-expect-error: mock private method
  const aggregateMock = jest.spyOn(etablissementService, "aggregate");
  //@ts-expect-error: mock private method
  jest
    .spyOn(etablissementService, "scoreEtablisement")
    .mockImplementation(() => 1);
  //@ts-expect-error: mock
  jest.spyOn(FormaterHelper, "formatData").mockImplementation((data) => data);

  describe("getVersements", () => {
    const getVersementsBySiretMock = jest.spyOn(
      versementsService,
      "getVersementsBySiret"
    );
  ...
  describe("getSubventions()", () => {
    const getDemandesByEtablissementMock = jest.spyOn(
      subventionsService,
      "getDemandesByEtablissement"
    );

    it("should call DemandeSubventionService.getByAssociation()", async () => {
      getDemandesByEtablissementMock.mockImplementationOnce(
        () => new Flux<SubventionsFlux>()
      );
  ...
  describe("getDocuments", () => {
    const getDocumentBySiretMock = jest.spyOn(
      documentsService,
      "getDocumentBySiret"
    );
  ...
  describe("getEtablissementsBySiren", () => {
    it("should call toSimplifiedEtablissement", async () => {
      ...
      toSimplifiedEtablissementMock.mockImplementationOnce(
