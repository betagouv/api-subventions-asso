...
export default class AdminStructureParser {
    ...
    public static parseXls(content: Buffer): AdminStructureEntity[] {
        const rows = GenericParser.xlsxParse(content)[0].data.slice(1) as string[][]; // single page
    ...
    private static exportNameToAgentType(value: string) {
        ...
        throw new BadRequestError(
            `Valeur de première colonne "${value}" non conforme, mettre à jour \`csvNameToAgentType\` dans \`admin-structure.parser\` ou la valeur dans le document source.`,
        );
    }
    ...
    private static exportNameToTerritoryLevel(value: string): AdminTerritorialLevel | undefined {
        ...
        console.warn(
            `Valeur de seconde colonne "${value}" non conforme, mettre à jour \`csvNameToTerritoryLevel\` dans \`admin-structure.parser\` ou la valeur dans le document source.`,
        );
    }
}
