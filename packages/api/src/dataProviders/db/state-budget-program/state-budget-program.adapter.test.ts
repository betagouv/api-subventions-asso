...
import stateBudgetProgramAdapter from "./state-budget-program.adapter";
...
describe("StateBudgetProgram Port", () => {
    describe("replace()", () => {
        it("should call deleteMany", async () => {
            await stateBudgetProgramAdapter.replace(STATE_BUDGET_PROGRAM_ENTITIES);
    ...
    describe("findByCode()", () => {
        it("should return the matching StateBudgetProgramDbo", async () => {
            ...
            const program = await stateBudgetProgramAdapter.findByCode(code);
