...
export class UniteLegalNamesPort extends MongoRepository<UniteLegalNameDbo>{
    ...
    async findOneBySiren(siren: Siren) {
        ...
        const dbo = await cursor.next() as UniteLegalNameDbo;
        await cursor.close()
    ...
    insert(entity: UniteLegalNameEntity) {
        ...
        return this.insertSirenStack.addOperation(UniteLegalNameAdapter.toDbo(entity)).catch(error => {
            if (error instanceof MongoServerError && error.code === "E11000") throw new DuplicateIndexError(error.message, entity);
            return error;
        });
    }
}

const uniteLegalNamesPort = new UniteLegalNamesPort();

export default uniteLegalNamesPort;