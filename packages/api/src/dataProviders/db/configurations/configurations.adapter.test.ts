import configurationsAdapter from "./configurations.adapter";

describe("Configuration Port", () => {
    // @ts-expect-error collection is private methods for getter
    const collectionMock: jest.SpyInstance<unknown> = jest.spyOn(configurationsAdapter, "collection", "get");
    ...
    describe("upsert", () => {
        it("should be call updateOne of collection", async () => {
            ...
            configurationsAdapter.upsert("TEST", {
                data: "test-data",
                name: "TEST",
            });
