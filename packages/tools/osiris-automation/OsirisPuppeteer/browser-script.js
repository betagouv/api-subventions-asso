module.exports = (year) => {
    ...
    window.SaveByteArray = (reportName, byte) => { // Use this because all files have the same name
        return holdSaveByteArray((window.currentExportName || "NO_CURRENT_EXPORT_NAME") + '-' + reportName, byte);
    }

    window.onFileDowloadEnded = () => { // Is call by node when dowload is ended
    ...
    async function stepTypeFederation(name) {
        ...
        const download = async (name) => {
            ...
            if (domHelper.isInDepassement()) {
                console.error("EXPORT to big");
                return // Export to BIG
            }
